...
function openPage(name){
  ...
  if(name==="coupons")loadCoupons();if(name==="rates")loadRates();if(name==="reports")loadReports();if(name==="business")loadBusiness();
}
...
async function loadProducts(){
 ...
 $("#productsTable").innerHTML=ps.length?`<table class="table"><thead><tr><th>Product</th><th>Price</th><th>Stock</th><th>Weight</th><th>Discount</th><th>Actions</th></tr></thead><tbody>${ps.map(p=>`<tr><td><div class="admin-product-cell">${p.image?`<img src="${p.image}" alt="">`:""}<div><strong>${esc(p.name)}</strong><br><span class="muted">${esc(p.purity||"")} • ${esc(p.category)}</span></div></div></td><td>${money(p.price)}${p.oldPrice?`<br><s class="muted">${money(p.oldPrice)}</s>`:""}</td><td><strong class="${Number(p.stock||0)<=3?"low-stock":""}">${Number(p.stock||0)}</strong></td><td>${Number(p.weight||0)}g</td><td>${Number(p.discount||0)}%</td><td><button class="btn btn-small btn-gold" data-edit-product="${p.id}">Edit</button> <button class="btn btn-small btn-danger" data-delete-product="${p.id}">Delete</button></td></tr>`).join("")}</tbody></table>`:'<div class="empty">No products.</div>';
...
productImageFile.onchange=async()=>{const file=productImageFile.files[0];if(!file)return;try{toast("Uploading photo...");const image=await resizeProductImage(file),uploaded=await api("/api/admin/images",{method:"POST",body:JSON.stringify({image})});showProductImage(uploaded.url);toast(uploaded.storage==="cloudinary"?"Photo uploaded to Cloudinary":"Photo ready") }catch(err){productImageFile.value="";toast(err.message)}};
...
async function loadCustomers(){const cs=await api("/api/admin/customers");$("#customersTable").innerHTML=cs.length?`<table class="table"><thead><tr><th>Name</th><th>Email</th><th>Phone</th><th>Registered</th><th>Access</th></tr></thead><tbody>${cs.map(c=>`<tr><td><strong>${esc(c.name)}</strong></td><td>${esc(c.email)}</td><td>${esc(c.phone||"-")}</td><td>${new Date(c.createdAt).toLocaleDateString()}</td><td><button class="btn btn-small ${c.blocked?"btn-gold":"btn-danger"}" data-block-user="${c.id}" data-blocked="${c.blocked}">${c.blocked?"Unblock":"Block"}</button></td></tr>`).join("")}</tbody></table>`:'<div class="empty">No registered customers yet.</div>';$$("[data-block-user]").forEach(b=>b.onclick=async()=>{await api("/api/admin/customers/"+b.dataset.blockUser+"/block",{method:"PUT",body:JSON.stringify({blocked:b.dataset.blocked!=="true"})});loadCustomers();toast("Customer access updated")})}
...
async function loadCoupons(){const cs=await api("/api/admin/coupons");$("#couponsList").innerHTML=cs.length?`<div class="offer-list">${cs.map(c=>`<div class="offer-card"><div><h4>${esc(c.code)}</h4><p>${c.type==="flat"?money(c.value):c.value+"%"} discount ${c.expiresAt?"• Expires "+esc(c.expiresAt):""}</p></div><button class="btn btn-small btn-danger" data-delete-coupon="${c.id}">Delete</button></div>`).join("")}</div>`:'<div class="empty">No coupons.</div>';$$("[data-delete-coupon]").forEach(b=>b.onclick=async()=>{await api("/api/admin/coupons/"+b.dataset.deleteCoupon,{method:"DELETE"});loadCoupons();toast("Coupon deleted")})}
$("#couponForm").onsubmit=async e=>{e.preventDefault();await api("/api/admin/coupons",{method:"POST",body:JSON.stringify(Object.fromEntries(new FormData(e.target)))});e.target.reset();loadCoupons();toast("Coupon created")};
async function loadRates(){const r=await api("/api/admin/rates"),f=$("#ratesForm");Object.keys(r).forEach(k=>{if(f.elements[k])f.elements[k].value=r[k]||""});$("#ratesMsg").textContent=r.updatedAt?"Last updated: "+new Date(r.updatedAt).toLocaleString():"Rates not updated yet."}
$("#ratesForm").onsubmit=async e=>{e.preventDefault();await api("/api/admin/rates",{method:"PUT",body:JSON.stringify(Object.fromEntries(new FormData(e.target)))});loadRates();toast("Metal rates updated")};
async function loadReports(){const r=await api("/api/admin/reports");$("#reportStats").innerHTML=`<div class="stat gold"><small>LOW STOCK</small><strong>${r.lowStock.length}</strong></div><div class="stat"><small>BEST SELLERS</small><strong>${r.bestSellers.length}</strong></div>`;$("#reportsPanel").innerHTML=`<h3>Best Sellers</h3>${r.bestSellers.length?`<table class="table"><tr><th>Product</th><th>Units</th></tr>${r.bestSellers.map(x=>`<tr><td>${esc(x.name)}</td><td>${x.qty}</td></tr>`).join("")}</table>`:'<div class="empty">No sales yet.</div>'}<h3>Low Stock</h3>${r.lowStock.map(p=>`<p>${esc(p.name)} — <b>${p.stock}</b> left</p>`).join("")||'<div class="empty">No low-stock products.</div>'}`}
$("#exportOrders").onclick=async e=>{e.preventDefault();const res=await fetch(e.currentTarget.href,{headers:{"x-admin-key":A.key}}),blob=await res.blob(),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="orders.csv";a.click();URL.revokeObjectURL(a.href)};
async function loadBusiness(){const d=await api("/api/admin/business");$("#businessPanel").innerHTML=`<h3>Pending Reviews</h3>${d.reviews.map(r=>`<div class="offer-card"><div><b>${esc(r.name)} • ${r.rating}/5</b><p>${esc(r.comment)}</p></div><button class="btn btn-small btn-gold" data-review="${r.id}">${r.approved?"Approved":"Approve"}</button></div>`).join("")||'<div class="empty">No reviews.</div>'}<h3>Appointments</h3>${d.appointments.map(x=>`<p><b>${esc(x.name||"")}</b> • ${esc(x.phone||"")} • ${esc(x.date||"")} • ${esc(x.status)}</p>`).join("")||'<div class="empty">No appointments.</div>'}<h3>Custom Jewellery Requests</h3>${d.customRequests.map(x=>`<p><b>${esc(x.name||"")}</b> • ${esc(x.phone||"")}<br>${esc(x.details||x.message||"")}</p>`).join("")||'<div class="empty">No custom requests.</div>'}`;$$("[data-review]").forEach(b=>b.onclick=async()=>{await api("/api/admin/reviews/"+b.dataset.review,{method:"PUT",body:JSON.stringify({approved:true})});loadBusiness();toast("Review approved")})}
